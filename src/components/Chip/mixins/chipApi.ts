...
export function useChipClassNames(props: Readonly<TChipOptionProps>, attrs: TRecord): TRecord {
  ...
  return {
    ...
    'chip-rounded-pill': props.pill,
    [`chip-${props.color}`]: enableColor && !props.outlined,
    [`chip-outline-${props.color}`]: enableColor && props.outlined,
    [props.activeClass as string]: props.activeClass && props.active === true && !props.disabled,
    [`${cssPrefix}link select-none`]: clickable,
    'inline-flex': true,
    'items-center': true,
    'max-w-full': true,
