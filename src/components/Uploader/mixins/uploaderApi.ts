...
function isDragAndDropSupported(): boolean {
  ...
  return (
    ...
    // @ts-expect-error: check touch support
...
function isFileSizeValid(
  ...
  if (source.size <= 0 || source.size > parseFloat(limit) * _1Mb) {
    const size = Helper.roundNumber(source.size / _1Mb, 2);
  ...
  return h(
    ...
    [
      h('canvas', {
        ...
        onDrop: (e) => {
          onFileDrop(e, emit, props, containerRef, canvasRef, mimeTypes, fileBag, draggingOver);
        },
        onClick: () => {
...
function renderImageUploader(
  ...
  if (isPreviewSupported()) {
    ...
    return [createInputFile(emit, props, containerRef, canvasRef, inputRef, mimeTypes, fileBag)];
  }
}
...
export function useSetupImageUploader(
  props: Readonly<TImageUploaderOptionProps>,
  expose: <Exposed extends TRecord = TRecord>(exposed?: Exposed) => void,
  containerRef: Ref<HTMLElement | undefined>,
  canvasRef: Ref<HTMLCanvasElement | undefined>,
  fileBag: Reactive<TFileBag>
): void {
  ...
  function onResize() {
    if (containerRef.value && canvasRef.value) {
      ...
      fileBag.image && drawImage(fileBag.image, canvasRef, containerRef);
    }
  }
