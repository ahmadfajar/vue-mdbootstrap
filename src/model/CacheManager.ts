import type { ICacheItem, ICacheManager, TRawCacheItem } from '@/model/types';
...
declare type TCacheStorage = Map<string, TCacheInternal>;
...
export const CacheManager: ICacheManager = {
  ...
  clear(): void {
    (this._cacheItems as TCacheStorage).clear();
  },
  deleteItem(key: string): boolean {
    return (this._cacheItems as TCacheStorage).delete(key);
  },
  ...
  getItem(key: string): ICacheItem | undefined {
    const ret = (this._cacheItems as TCacheStorage).get(key);
  ...
  save(item: TRawCacheItem | ICacheItem): void {
    ...
    if ('getKey' in item && Helper.isFunction(item.getKey)) {
      ...
      (this._cacheItems as TCacheStorage).set(it.getKey(), {
        value: it.getValue(),
        expiry: it.expiredAt(),
        hits: it.getHits(),
      });
      ...
      (this._cacheItems as TCacheStorage).set(it.key, {
        value: it.value,
  ...
  size(): number {
    return (this._cacheItems as TCacheStorage).size;
  },
};
